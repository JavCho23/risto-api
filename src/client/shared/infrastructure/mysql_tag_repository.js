...
class MySqlTagRepository {
    async listTag(id_meal, id_category) {
        const data = await db.doQuery(
            "SELECT tag.name FROM tag INNER JOIN tagger ON tag.id_tag = tagger.id_tag INNER JOIN meal ON meal.id_meal = tagger.id_meal WHERE meal.id_meal = ? AND id_category = ?",
