...
class MySqlCategoryRepository {
    ...
    async listLocalCategories(localId){

        const data = await db.doQuery(
            `SELECT tag.name, count(tag.name) as total 
            FROM qualification 
            INNER JOIN score ON score.id_qualification = qualification.id_qualification  
            INNER JOIN tag on score.id_tag = tag.id_tag 
            WHERE qualification.id_local = ? AND qualification.state = 1 GROUP BY tag.name`,
            localId
        );
        if (data.length == 0) ;
